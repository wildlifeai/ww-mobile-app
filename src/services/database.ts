...
export const userOperations = {
   ...
   * Update user profile
   */
  updateProfile: async (updates: TablesUpdate<'users'>): Promise<Tables<'users'>> => {
    const { data: { user } } = await supabase.auth.getUser();
    if (!user) throw new Error('No authenticated user');
...
export const deviceOperations = {
   ...
   * Create new device
   */
  createDevice: async (device: TablesInsert<'devices'>): Promise<Tables<'devices'>> => {
    const { data, error } = await supabase
      .from('devices')
      .insert(device)
      .select('*')
...
export const deploymentOperations = {
   ...
   */
  createDeployment: async (deployment: TablesInsert<'deployments'>): Promise<Tables<'deployments'>> => {
    const { data, error } = await supabase
      .from('deployments')
      .insert(deployment)
      .select('*')
...
export const subscriptions = {
   ...
   */
  subscribeToProjectDeployments: (projectId: string, callback: (payload: any) => void) => {
    const channel = supabase
