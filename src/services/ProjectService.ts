...
class ProjectService {
	...
	async getUserProjects(organisationId: string): Promise<ProjectWithDetails[]> {
		try {
			console.log(
				"📂 Reading projects from local database for org:",
				organisationId,
			)

			// STEP 1: Read from local database (ALWAYS, even offline)
			const localProjects = await this.db.getProjectsByOrganisation(
				organisationId,
			)
		...
		try {
			...
			if (!viewError && viewData) {
				...
				for (const project of viewData) {
					...
					console.log(
						`💾 Saving project "${project.name}" with org_id=${project.organisation_id}`,
					)
	...
	async updateProject(
		...
		try {
			...
			if (updates.deleted_at !== undefined)
				dbUpdates.status = updates.deleted_at ? "inactive" : "active"
	...
	async deleteProject(projectId: string): Promise<void> {
		try {
			...
			console.log(
				...
				pendingOps.filter((op) => op.operation_type === "DELETE_PROJECT")
					.length,
			)
	...
	async getProjectMembers(
		...
		return members.map((member: any) => ({
			...
			role: {
				...
				description:
					member.role_value === "project_admin"
	...
	async addProjectMember(
		...
		if (error) {
			...
			if (error.message.includes("same organisation")) {
				throw new Error(
					"User must belong to the same organisation as the project",
				)
			}
	 ...
	 */
	private mapDatabaseProjectToDetails = (
