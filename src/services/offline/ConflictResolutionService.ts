...
export class ConflictResolutionService {
  ...
  async detectConflicts(
    ...
    if (this.hasDataModificationConflict(serverData, localData)) {
      conflicts.push(this.createConflictResolution(
        serverData,
        localData,
        'data_modification'
      ));
  ...
  async resolveConflicts(
    ...
    for (const conflict of conflicts) {
      ...
      const resolutionStrategy = strategy || this.determineOptimalStrategy(conflict);
   ...
   */
  private hasDataModificationConflict(serverData: any, localData: any): boolean {
  ...
  private async hasPermissionConflict(
    ...
    if (serverData?.organisation_id && serverData.organisation_id !== user.organisation_id) {
  ...
  private createConflictResolution(
    ...
    return {
      ...
      needs_user_resolution: this.needsUserResolution(conflictType, serverData, localData)
    };
  }
   ...
   */
  private async resolveServerWins(conflict: ConflictResolution): Promise<ConflictResolution> {
    return {
      ...conflict,
   ...
   */
  private async resolveLocalWins(conflict: ConflictResolution): Promise<ConflictResolution> {
    return {
      ...conflict,
  ...
  private async resolveMerge(conflict: ConflictResolution): Promise<ConflictResolution> {
    const mergedData = this.mergeData(conflict.server_version, conflict.local_version);
   ...
   */
  private async resolveUserChoice(conflict: ConflictResolution): Promise<ConflictResolution> {
    ...
    console.warn(`Conflict needs user resolution: ${conflict.id} (${conflict.conflict_type})`);
  ...
  private getMostRecentTimestamp(serverData: any, localData: any): string {
    const serverTime = serverData?.updated_at ? new Date(serverData.updated_at) : new Date(0);
    const localTime = localData?.updated_at ? new Date(localData.updated_at) : new Date(0);
...
export const getConflictResolutionService = (databaseService: DatabaseService): ConflictResolutionService => {
