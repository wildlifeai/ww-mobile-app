...
export class SyncService {
	...
	async startSync(user: User): Promise<SyncStatus> {
		...
		try {
			...
			const pendingOperations = await this.offlineService.getOperationsForSync(
				user,
			)
			this.updateSyncStatus({
			...
			this.updateSyncStatus({
				is_syncing: false,
				sync_errors: [
					...(this.currentSyncStatus.sync_errors || []),
					error.message,
				],
			})
			throw error
	 ...
	 */
	private async syncOfflineOperationsToServer(
			...
			try {
				...
				this.updateSyncStatus({
					failed_operations_count:
						(this.currentSyncStatus.failed_operations_count || 0) + 1,
				})
			}
		}
	}
	...
	private async syncServerChangesToLocal(user: User): Promise<void> {
		try {
			...
			const serverChanges = await this.fetchIncrementalServerChanges(
				user,
				lastSyncTime,
			)
	 ...
	 */
	private async detectServerConflict(
		...
		try {
			...
			console.error(
				`Failed to detect server conflict for operation ${operation.id}:`,
				error,
			)
			return null
		}
	...
	private async detectLocalConflict(
		...
		try {
			...
			// Check for data modification conflicts
			if (
				this.offlineService.detectPotentialConflict(serverChange, localState)
			) {
				return {
			...
			console.error(
				`Failed to detect local conflict for change ${serverChange.id}:`,
				error,
			)
			return null
		}
	...
	private hasDataModificationConflict(
		...
		return this.offlineService.detectPotentialConflict(
			serverState,
			operation.data,
		)
	}
	...
	getUnresolvedConflicts(): ConflictResolution[] {
		return Array.from(this.conflictResolutions.values()).filter(
			(conflict) => conflict.needs_user_resolution,
		)
	}
	...
	private async getServerResourceState(
		operation: OfflineOperation,
		user: User,
	): Promise<any> {
	...
	private async fetchIncrementalServerChanges(
		user: User,
		lastSyncTime: Date,
	): Promise<any[]> {
	...
	private async applyServerChangeLocally(
		change: any,
		user: User,
	): Promise<void> {
