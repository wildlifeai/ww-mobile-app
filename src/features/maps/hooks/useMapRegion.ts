...
export const useMapRegion = (initialRegion?: MapRegion): UseMapRegionReturn => {
  const [region, setRegionState] = useState<MapRegion>(initialRegion || DEFAULT_REGION);
   ...
   */
  const centerOnLocation = useCallback((location: UserLocation, zoomLevel?: number) => {
    const newRegion: MapRegion = {
      latitude: location.latitude,
      longitude: location.longitude,
      latitudeDelta: zoomLevel ? calculateDeltaFromZoom(zoomLevel) : region.latitudeDelta,
      longitudeDelta: zoomLevel ? calculateDeltaFromZoom(zoomLevel) : region.longitudeDelta,
    };
