...
describe('Simulated Transport Resiliency', () => {
  ...
  // The app and the firmware do not always agree on how many op indices exist
  // (the app once carried op32 before any build shipped it). Asking for one the
  // running build does not have used to match neither success nor failure, so
  // the command sat for its full 8s timeout and then retried for another 8. On
  // the bench that put 16s of dead time on entering Capture Preview and blocked
  // the capture queued behind it, so the flow looked like it had done nothing.
  test('an op index the firmware lacks fails fast, without waiting out the timeout', async () => {
    const mockWrite = transport.writeToDevice as jest.Mock;

    mockWrite.mockImplementation(async (_periph, payload) => {
      expect(payload).toBe('AI getop 40');
      rxRouter.handleIncomingBytes(DEVICE_ID, Buffer.from(
    ...
    await expect(
      bleTransport.enqueue(() => runCommandPipeline(peripheral, () => commandRegistry.getop(40)))
    ...
    const getopWrites = mockWrite.mock.calls.filter((c: any[]) => c[1] === 'AI getop 40');
