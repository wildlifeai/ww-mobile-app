...
const updateDeviceLoRaWANStatusHelper = (
	device: DeploymentDevice,
	...
	reducers: {
		setDeployments: (state, action: PayloadAction<SetDeploymentsPayload>) => {
			...
			if (authContext.userRole === "ww_admin") {
				...
				state.deployments = deployments.filter(
					(d) => d.organisation_id === authContext.currentOrgId,
				)
			}

			// Update active deployments
			state.activeDeployments = state.deployments.filter(
				(d) => d.status === "active",
			)
		...
		createDeployment: (
			state,
			action: PayloadAction<CreateDeploymentPayload>,
		) => {
		...
		updateDeployment: (
			state,
			action: PayloadAction<UpdateDeploymentPayload>,
		) => {
		...
		deleteDeployment: (
			state,
			action: PayloadAction<DeleteDeploymentPayload>,
		) => {
		...
		updateDeviceLoRaWANStatus: (
			...
			state.deployments.forEach((deployment) => {
				const deviceIndex = deployment.devices.findIndex(
					(d) => d.id === deviceId,
				)
				if (deviceIndex !== -1) {
			...
			if (state.currentDeployment) {
				const deviceIndex = state.currentDeployment.devices.findIndex(
					(d) => d.id === deviceId,
				)
				if (deviceIndex !== -1) {
					state.currentDeployment.devices[deviceIndex] =
						updateDeviceLoRaWANStatusHelper(
							state.currentDeployment.devices[deviceIndex],
							status,
						)
				}
		...
		bulkUpdateDeviceStatus: (
			state,
			action: PayloadAction<Record<string, LoRaWANStatus>>,
		) => {
		...
		removeDeviceFromDeployment: (
			state,
			action: PayloadAction<RemoveDevicePayload>,
		) => {
...
export const selectActiveDeployments = (state: {
	deployments: DeploymentsState
}) => state.deployments.activeDeployments

export const selectCurrentDeployment = (state: {
	deployments: DeploymentsState
}) => state.deployments.currentDeployment

export const selectDeploymentsLoading = (state: {
	deployments: DeploymentsState
}) => state.deployments.loading

export const selectDeploymentsError = (state: {
	deployments: DeploymentsState
}) => state.deployments.error

export const selectDeviceStatusUpdates = (state: {
	deployments: DeploymentsState
}) => state.deployments.deviceStatusUpdates

// Organisation and project scoped selectors
export const selectDeploymentsByProject =
	(projectId: string) => (state: { deployments: DeploymentsState }) =>
		state.deployments.deployments.filter((d) => d.project_id === projectId)

export const selectDeploymentsByOrganisation =
	(organisationId: string) => (state: { deployments: DeploymentsState }) =>
		state.deployments.deployments.filter(
			(d) => d.organisation_id === organisationId,
		)

export const selectUserDeployments =
	(userId: string) => (state: { deployments: DeploymentsState }) =>
		state.deployments.deployments.filter((d) => d.created_by === userId)

// LoRaWAN status selectors
export const selectDeviceLoRaWANStatus =
	(deviceId: string) => (state: { deployments: DeploymentsState }) =>
		state.deployments.deviceStatusUpdates[deviceId]

export const selectLowBatteryDevices =
	(threshold: number = 20) =>
...
export const selectHighStorageDevices =
	(threshold: number = 80) =>
	(state: { deployments: DeploymentsState }) =>
		Object.entries(state.deployments.deviceStatusUpdates)
