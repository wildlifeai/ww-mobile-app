  ...
  endpoints: (builder) => ({
    ...
    getProjects: builder.query<PaginatedResponse<any>, ProjectQuery>({
      query: ({ organisation_id, user_role, user_id, status, created_by }) => ({
        url: '/projects',
        params: {
          organisation_id: user_role === 'ww_admin' ? undefined : organisation_id,
          status,
    ...
    updateProject: builder.mutation<APIResponse<any>, { id: string; updates: Partial<any> }>({
      query: ({ id, updates }) => ({
        url: `/projects/${id}`,
        method: 'PATCH',
        body: updates,
    ...
    getDeployments: builder.query<PaginatedResponse<any>, DeploymentQuery>({
      query: ({ organisation_id, user_role, project_id, status, include_devices }) => ({
        url: '/deployments',
        params: {
          organisation_id: user_role === 'ww_admin' ? undefined : organisation_id,
          project_id,
    ...
    createDeployment: builder.mutation<APIResponse<any>, CreateDeploymentRequest>({
      query: (deployment) => ({
        url: '/deployments',
        method: 'POST',
        body: deployment,
    ...
    updateDeployment: builder.mutation<APIResponse<any>, { id: string; updates: Partial<any> }>({
      query: ({ id, updates }) => ({
        url: `/deployments/${id}`,
        method: 'PATCH',
        body: updates,
    ...
    updateDeviceStatus: builder.mutation<APIResponse<void>, LoRaWANWebhookPayload>({
      query: (payload) => ({
    ...
    createOrganisation: builder.mutation<APIResponse<any>, { name: string; description?: string }>({
      query: (org) => ({
    ...
    updateUserStatus: builder.mutation<APIResponse<any>, { 
      id: string; 
    ...
    loginUser: builder.mutation<APIResponse<any>, { email: string; password: string }>({
      query: (credentials) => ({
        url: '/auth/login',
        method: 'POST',
        body: credentials,
    ...
    setupRealtimeSubscription: builder.query<any, { table: string; organisation_id?: string }>({
