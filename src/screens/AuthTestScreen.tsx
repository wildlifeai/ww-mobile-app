...
export const AuthTestScreen: React.FC = () => {
  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      if ((authResponse as any).isPendingConfirmation) {
        addTestResult(`📧 Registration successful! Please check your email to confirm your account.`);
  ...
  const runConnectivityTests = async () => {
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : 'API tests failed';
      ...
      <Card style={{ marginBottom: 16 }}>
        ...
        <Card.Content>
          <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8, marginBottom: 8 }}>
            <Chip icon={isLoggedIn ? 'check' : 'close'} mode={isLoggedIn ? 'flat' : 'outlined'}>
          ...
          {user && (
            <View>
              ...
              <Text><Text style={{ fontWeight: 'bold' }}>Username:</Text> {user.username}</Text>
              <Text><Text style={{ fontWeight: 'bold' }}>ID:</Text> {user.id}</Text>
              <Text><Text style={{ fontWeight: 'bold' }}>Confirmed:</Text> {user.confirmed ? 'Yes' : 'No'}</Text>
            </View>
          )}
        </Card.Content>
      </Card>
