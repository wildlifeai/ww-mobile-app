...
describe('ProjectService Integration Tests', () => {
  ...
  beforeAll(async () => {
    ...
    // Grant roles
    await grantUserRole(org1AdminId, testRoles.projectAdmin, 'organisation', org1Id);
    await grantUserRole(org1MemberId, testRoles.projectMember, 'organisation', org1Id);
    await grantUserRole(org2AdminId, testRoles.projectAdmin, 'organisation', org2Id);
  ...
  describe('Organisation Isolation', () => {
    ...
    beforeEach(async () => {
      ...
      await signInTestUser(testUsers.org2Admin.email, testUsers.org2Admin.password);
  ...
  describe('CRUD Operations', () => {
    ...
    it('should prevent project update by non-admin', async () => {
      ...
      await testSupabase.auth.signOut();
      await signInTestUser(testUsers.org1Member.email, testUsers.org1Member.password);
  ...
  describe('Member Management', () => {
    ...
    afterEach(async () => {
      ...
      await adminSupabase.from('project_members').delete().eq('project_id', projectId);
      await adminSupabase.from('projects').delete().eq('id', projectId);
    });

    it('should add member to project', async () => {
      // Add org1 member to project
      await ProjectService.addProjectMember(projectId, org1MemberId, roleIds.projectMember);
    ...
    it('should prevent cross-org member assignment', async () => {
      ...
      await expect(
        ProjectService.addProjectMember(projectId, org2AdminId, roleIds.projectMember)
  ...
  describe('Computed Fields', () => {
    ...
    it('should return correct member_count', async () => {
      // Add 2 members
      await ProjectService.addProjectMember(projectId, org1MemberId, roleIds.projectMember);
      await ProjectService.addProjectMember(projectId, wwAdminId, roleIds.projectMember);
  ...
  describe('Error Handling', () => {
    ...
    it('should handle cross-org assignment attempts', async () => {
      ...
      await expect(
        ProjectService.addProjectMember(project.id, org2AdminId, roleIds.projectMember)
      ).rejects.toThrow(/same organisation/i);
