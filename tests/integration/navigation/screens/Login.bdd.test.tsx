...
import { renderWithProviders, createTestStore } from '../../../setup/utils/testUtils';
import {
  mockAuthSuccess,
  mockAuthError,
  resetSupabaseMocks,
  ...
  test('User Story: Successful Login', async () => {
    ...
    await createUserStory('Successful User Login')
      ...
      .scenario('Valid credentials provided')
        ...
        .then('I should be authenticated successfully', AuthActions.systemAuthenticatesUser())
      .executeAll();
  });
  ...
  test('User Story: Login Validation', async () => {
    ...
    await createUserStory('Login Form Validation')
      ...
      .scenario('Empty form submission')
        .given('I am on the login screen', AuthActions.userIsOnLoginScreen)
        .when('I submit the form without entering any information', AuthActions.userSubmitsLoginForm)
        .then('I should see an email required error', AuthActions.systemShowsValidationError(ValidationMessages.emailRequired))
        .and('I should see a password required error', AuthActions.systemShowsValidationError(ValidationMessages.passwordRequired))
      .executeAll();
  });
  ...
  test('User Story: Email Format Validation', async () => {
    ...
    await createUserStory('Email Format Validation')
      ...
      .scenario('Invalid email format')
        ...
        .then('I should see an invalid email error', AuthActions.systemShowsValidationError(ValidationMessages.emailInvalid))
      .executeAll();
  });
  ...
  test('User Story: Password Length Validation', async () => {
    ...
    await createUserStory('Password Length Validation')
      ...
      .scenario('Password too short')
        ...
        .and('I submit the form', AuthActions.userSubmitsLoginForm)
        .then('I should see a password too short error', AuthActions.systemShowsValidationError(ValidationMessages.passwordTooShort))
      .executeAll();
  });
  ...
  test('User Story: Navigation to Registration', async () => {
    ...
    await createUserStory('Navigate to Registration')
      ...
      .scenario('Click registration link')
        .given('I am on the login screen', AuthActions.userIsOnLoginScreen)
        .when('I click on the registration link', AuthActions.userNavigatesToRegister)
        .then('I should be navigated to the registration screen', AuthActions.systemNavigatesToScreen('Register'))
      .executeAll();
  });
  ...
  test('User Story: Navigation to Forgot Password', async () => {
    ...
    await createUserStory('Navigate to Forgot Password')
      ...
      .scenario('Click forgot password link')
        .given('I am on the login screen', AuthActions.userIsOnLoginScreen)
        .when('I click on the forgot password link', AuthActions.userNavigatesToForgotPassword)
        .then('I should be navigated to the forgot password screen', AuthActions.systemNavigatesToScreen('ForgotPassword'))
      .executeAll();
  });
    ...
    await createUserStory('Remember Me Feature')
      ...
      .scenario('Login with remember me checked')
        ...
        .then('My credentials should be saved for next time', async () => {
          ...
          expect(AsyncStorage.setItem).toHaveBeenCalledWith('rememberedEmail', TestData.validUser.email);
    ...
    await createUserStory('Form Interaction')
      ...
      .scenario('Field properties and behavior')
        ...
        .when('I look at the email field', () => {
          const emailInput = require('@testing-library/react-native').screen.getByLabelText('Email');
          expect(emailInput.props.keyboardType).toBe('email-address');
          expect(emailInput.props.autoCapitalize).toBe('none');
          expect(emailInput.props.textContentType).toBe('emailAddress');
        })
        .and('I look at the password field', () => {
          const passwordInput = require('@testing-library/react-native').screen.getByLabelText('Password');
          expect(passwordInput.props.secureTextEntry).toBe(true);
        })
        .then('the fields should have appropriate keyboard and input settings', () => {
