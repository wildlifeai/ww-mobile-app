...
export class ScenarioBuilder {
  ...
  private _steps: Array<{ type: 'given' | 'when' | 'then' | 'and', description: string, action: () => void | Promise<void> }> = [];
  ...
  given(description: string, action: () => void | Promise<void> = () => {}): ScenarioBuilder {
    this._steps.push({ type: 'given', description, action });
    return this;
  }

  when(description: string, action: () => void | Promise<void> = () => {}): ScenarioBuilder {
    this._steps.push({ type: 'when', description, action });
    return this;
  }

  then(description: string, action: () => void | Promise<void> = () => {}): ScenarioBuilder {
    this._steps.push({ type: 'then', description, action });
    return this;
  }

  and(description: string, action: () => void | Promise<void> = () => {}): ScenarioBuilder {
