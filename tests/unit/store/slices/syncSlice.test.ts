...
describe('syncSlice', () => {
  ...
  describe('markEntityError', () => {
    ...
    it('should maintain last 10 errors only', () => {
      ...
      for (let i = 0; i < 15; i++) {
        state = syncReducer(
          state,
          markEntityPending({ entityType: 'projects', entityId: `project-${i}` })
        );
        state = syncReducer(
          state,
          markEntityError({
  ...
  describe('selectors', () => {
    ...
    it('selectEntitySyncStatus should return entity status', () => {
      ...
      const entityStatus = selectEntitySyncStatus(state, 'projects', 'project-1');
    ...
    it('selectEntitySyncStatus should return default for non-existent entity', () => {
      const state = { sync: initialState } as any;
      const entityStatus = selectEntitySyncStatus(state, 'projects', 'non-existent');
