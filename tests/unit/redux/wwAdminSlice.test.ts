...
describe("wwAdminSlice - Read-Only + Web Portal Architecture", () => {
	...
	describe("Initial State", () => {
		it("should return the initial state", () => {
			const state: WWAdminState = (
				store.getState() as { wwAdmin: WWAdminState }
			).wwAdmin
	...
	describe("Project Filtering Selectors", () => {
		...
		it("should select projects by organisation", () => {
			...
			expect(org1Projects.every((p) => p.organisation_id === "org-1")).toBe(
				true,
			)
		...
		it("should return empty array for non-existent organisation", () => {
			const state = store.getState() as { wwAdmin: WWAdminState }
			const nonExistentOrgProjects =
				selectProjectsByOrganisation("org-999")(state)
