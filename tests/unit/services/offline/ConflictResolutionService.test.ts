...
describe("ConflictResolutionService", () => {
	...
	describe("detectConflicts", () => {
		...
		it("should detect deletion conflicts", async () => {
			...
			expect(
				conflicts.some((c) => c.conflict_type === "deletion_conflict"),
			).toBe(true)
		})
		...
		it("should detect permission conflicts for cross-organisation access", async () => {
			...
			expect(conflicts.length).toBeGreaterThan(0)
			expect(
				conflicts.some((c) => c.conflict_type === "permission_conflict"),
			).toBe(true)
			expect(
				conflicts.some((c) => c.conflict_type === "data_modification"),
			).toBe(true)
			expect(
				conflicts.some(
					(c) => c.conflict_type === "organisation_boundary_conflict",
				),
			).toBe(true)
		})
	...
	describe("resolveConflicts", () => {
		it("should resolve conflicts with server wins strategy", async () => {
			...
			const resolvedData = await conflictResolutionService.resolveConflicts(
				[conflict],
				"server_wins",
			)
	...
	describe("conflict history and cleanup", () => {
		it("should retrieve conflict history", async () => {
			...
			const history = await conflictResolutionService.getConflictHistory(
				"project-1",
			)

			expect(history).toEqual(mockHistory)
			expect(mockDatabaseService.getConflictHistory).toHaveBeenCalledWith(
				"project-1",
			)
		})
	...
	describe("edge cases", () => {
		...
		it("should store conflict resolution even if storage fails", async () => {
			mockDatabaseService.storeConflictResolution.mockRejectedValue(
			...
			expect(async () => {
				await conflictResolutionService.resolveConflicts(
					[conflict],
					"server_wins",
				)
			}).not.toThrow()
