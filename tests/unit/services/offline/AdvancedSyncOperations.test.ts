...
describe("Advanced Sync Operations (Task 11.5)", () => {
	...
	describe("WW Admin Read-Only Operations (Updated Architecture)", () => {
		it("should handle CREATE_ORGANISATION operation for system-level operations", async () => {
			...
			expect(mockDatabaseService.markQueueItemCompleted).toHaveBeenCalledWith(
				operation.id,
			)
		})
	...
	describe("Batch Sync Operations", () => {
		it("should process operations in batches", async () => {
			...
			const operations: OfflineOperation[] = Array.from(
				{ length: 25 },
				(_, i) => ({
					id: `op-${i}`,
					type: "CREATE_PROJECT",
		...
		it("should throw error when offline", async () => {
			...
			await expect(
				offlineService.batchSyncOperations(operations),
			).rejects.toThrow("Network connection required for batch sync")
		})
	})
	...
	describe("Incremental Sync", () => {
		it("should sync operations since last timestamp", async () => {
			...
			const result = await offlineService.incrementalSync(
				testUser,
				lastSyncTime,
			)
		...
		it("should handle conflicts during incremental sync", async () => {
			...
			jest
				.spyOn(offlineService as any, "checkForConflicts")
				.mockResolvedValue(true)
			jest
				.spyOn(offlineService as any, "handleOperationConflict")
	...
	describe("Selective Sync", () => {
		it("should sync only specified operation types", async () => {
			...
			expect(
				mockDatabaseService.getQueueItemsByTypeAndPriority,
		...
		it("should throw error when offline", async () => {
			...
			await expect(
				offlineService.selectiveSync(testUser, ["CREATE_PROJECT"]),
			).rejects.toThrow("Network connection required for selective sync")
		})
	})
	...
	describe("Role-based Operation Validation", () => {
		it("should allow ww_admin to perform any operation", () => {
			...
			const canPerform = offlineService.canUserPerformOperation(
				wwAdminUser,
				operation,
			)
