...
describe('WWAdminOfflineService - Read-Only + Web Portal Architecture', () => {
  ...
  beforeEach(async () => {
    ...
    mockDatabaseService = {
      ...
      getProjectsByOrganisation: jest.fn().mockImplementation((orgId: string) => {
        return Promise.resolve(mockProjects.filter(p => p.organisation_id === orgId));
      }),
  ...
  describe('Cross-Organisation Project Visibility (Read-Only)', () => {
    ...
    it('should deny non-WW Admin access to all organisations', async () => {
      await expect(wwAdminService.getAllOrganisations(mockProjectAdmin))
        .rejects.toThrow('Unauthorized: Only WW Admins can access cross-organisation data');
    });

    it('should allow WW Admin to view projects by organisation', async () => {
      const projects = await wwAdminService.getProjectsByOrganisation(mockWWAdmin, 'org-1');
    ...
    it('should deny non-WW Admin access to cross-organisation projects', async () => {
      await expect(wwAdminService.getProjectsByOrganisation(mockProjectAdmin, 'org-2'))
        .rejects.toThrow('Unauthorized: Only WW Admins can access cross-organisation projects');
    });
  });
  ...
  describe('Project Overview and Statistics', () => {
    ...
    it('should deny non-WW Admin access to all projects', async () => {
      await expect(wwAdminService.getAllProjects(mockProjectAdmin))
    ...
    it('should deny non-WW Admin access to project statistics', async () => {
      await expect(wwAdminService.getProjectStatistics(mockProjectAdmin))
        .rejects.toThrow('Unauthorized: Only WW Admins can access project statistics');
    });
  });
  ...
  describe('Data Refresh and Cache Management', () => {
    ...
    it('should deny non-WW Admin data refresh operations', async () => {
      await expect(wwAdminService.refreshOrganisationProjects(mockProjectAdmin, 'org-1'))
        .rejects.toThrow('Unauthorized: Only WW Admins can refresh organisation data');
    });
    ...
    it('should deny non-WW Admin access to refresh all data', async () => {
      await expect(wwAdminService.refreshAllData(mockProjectAdmin))
  ...
  describe('WW Admin Access Validation', () => {
    ...
    it('should reject project_member role for WW Admin operations', async () => {
      ...
      await expect(wwAdminService.getAllOrganisations(projectMember))
        .rejects.toThrow(/Unauthorized.*WW Admin/);
    });
  ...
  describe('Integration with New wwAdminSlice Architecture', () => {
    ...
    it('should support permission validation expected by Redux selectors', async () => {
      ...
      await expect(wwAdminService.getAllProjects(mockWWAdmin)).resolves.toBeDefined();

      // Non-WW Admin should be denied
      await expect(wwAdminService.getAllProjects(mockProjectAdmin)).rejects.toThrow();
    });
  });
  ...
  describe('Error Handling and Edge Cases', () => {
    ...
    it('should handle missing projects for organisation', async () => {
      const projects = await wwAdminService.getProjectsByOrganisation(mockWWAdmin, 'non-existent-org');
