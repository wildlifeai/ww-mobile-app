...
describe('OfflineService', () => {
  ...
  describe('Network Monitoring', () => {
    it('should detect network status changes', async () => {
      ...
      const networkListener = (mockNetInfo.addEventListener as jest.Mock).mock.calls[0][0];
    ...
    it('should trigger sync when coming online', (done) => {
      const syncSpy = jest.spyOn(offlineService, 'syncPendingOperations').mockImplementation(() => {
  ...
  describe('Operations Queue', () => {
    ...
    it('should execute operations immediately when online', async () => {
      ...
      const executeSpy = jest.spyOn(offlineService, 'executeOperation').mockResolvedValue(true);
      await offlineService.queueOperation(operation);
  ...
  describe('Role-Based Sync Filtering', () => {
    ...
    describe('WW Admin Role', () => {
      ...
      it('should sync all organisations data for ww_admin', async () => {
        const operations = await offlineService.getOperationsForSync(wwAdminUser);
      ...
      it('should allow cross-organisation operations for ww_admin', () => {
        ...
        expect(offlineService.canUserPerformOperation(wwAdminUser, operation)).toBe(true);
      });
    });

    describe('Project Admin Role', () => {
      const projectAdminUser = { ...mockUser, role: 'project_admin' as UserRole };

      it('should sync only organisation data for project_admin', async () => {
        const operations = await offlineService.getOperationsForSync(projectAdminUser);
        expect(mockDatabaseService.getQueueItemsByOrganisation).toHaveBeenCalledWith(projectAdminUser.organisation_id);
      });
      ...
      it('should deny cross-organisation operations for project_admin', () => {
        ...
        expect(offlineService.canUserPerformOperation(projectAdminUser, operation)).toBe(false);
      });
    });

    describe('Project Member Role', () => {
      const projectMemberUser = { ...mockUser, role: 'project_member' as UserRole };

      it('should sync only accessible project data for project_member', async () => {
        const operations = await offlineService.getOperationsForSync(projectMemberUser);
        expect(mockDatabaseService.getQueueItemsByOrganisation).toHaveBeenCalledWith(projectMemberUser.organisation_id);
      });
      ...
      it('should deny organisation-level operations for project_member', () => {
        ...
        expect(offlineService.canUserPerformOperation(projectMemberUser, operation)).toBe(false);
      });
    });
  });
  ...
  describe('LoRaWAN Integration', () => {
    ...
    it('should cache LoRaWAN device status offline', async () => {
      ...
      await offlineService.updateDeviceLoRaWANStatus('device-1', deviceStatus, 'deployment-1');
      expect(mockDatabaseService.updateDeploymentLoRaWANStatus).toHaveBeenCalledWith('deployment-1', deviceStatus);
    });
  ...
  describe('Conflict Detection Foundation', () => {
    ...
    it('should detect potential conflicts based on timestamps', () => {
      const serverData = { id: 'item-1', updated_at: new Date('2025-01-01T12:00:00Z') };
      const localData = { id: 'item-1', updated_at: new Date('2025-01-01T11:00:00Z') };

      const hasConflict = offlineService.detectPotentialConflict(serverData, localData);
    ...
    it('should prepare conflict data for resolution', () => {
      const serverData = { id: 'item-3', name: 'Server Name', updated_at: new Date('2025-01-01T12:00:00Z') };
      const localData = { id: 'item-3', name: 'Local Name', updated_at: new Date('2025-01-01T11:00:00Z') };

      const conflictData = offlineService.prepareConflictResolution(serverData, localData);
  ...
  describe('Organisation Data Isolation', () => {
    ...
    it('should enforce organisation boundaries in offline operations', async () => {
      ...
      expect(offlineService.canUserPerformOperation(user, operation)).toBe(false);
    });
  ...
  describe('Service Cleanup', () => {
    it('should cleanup resources on destroy', async () => {
      ...
      (mockNetInfo.addEventListener as jest.Mock).mockReturnValue(unsubscribeMock);
