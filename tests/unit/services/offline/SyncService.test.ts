...
describe("SyncService", () => {
	...
	describe("Bidirectional Sync", () => {
		it("should start sync process with proper status updates", async () => {
			...
			expect(mockOfflineService.getOperationsForSync).toHaveBeenCalledWith(
				mockUser,
			)
		})

		it("should handle sync errors gracefully", async () => {
			mockOfflineService.getOperationsForSync.mockRejectedValue(
				new Error("Network error"),
			)

			await expect(syncService.startSync(mockUser)).rejects.toThrow(
				"Network error",
			)
	...
	describe("Role-Based Sync Filtering", () => {
		...
		it("should restrict project_member operations", async () => {
			...
			expect(mockDatabaseService.updateQueueItemRetry).toHaveBeenCalledWith(
				"op-restricted",
				1,
				"failed",
			)
	...
	describe("Error Handling and Recovery", () => {
		it("should handle database errors gracefully", async () => {
			...
			mockOfflineService.getOperationsForSync.mockResolvedValue([operation])
			mockDatabaseService.markQueueItemCompleted.mockRejectedValue(
		...
		it("should update retry count for failed operations", async () => {
			...
			jest
				.spyOn(syncService as any, "executeServerOperation")
