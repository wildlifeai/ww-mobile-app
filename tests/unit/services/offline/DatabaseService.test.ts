...
describe('DatabaseService', () => {
  ...
  describe('Organisation Multi-Tenancy', () => {
    ...
    it('should retrieve organisations by ID only', async () => {
      ...
      const result = await databaseService.getOrganisationById(sampleOrganisation.id);

      expect(mockDb.getFirstAsync).toHaveBeenCalledWith(
        expect.stringContaining('SELECT * FROM local_organisations WHERE id = ?'),
        [sampleOrganisation.id]
      );
  ...
  describe('User Role Management', () => {
    ...
    it('should retrieve user roles by organisation and user', async () => {
      ...
      expect(mockDb.getAllAsync).toHaveBeenCalledWith(
        expect.stringContaining('SELECT * FROM local_user_roles WHERE user_id = ? AND organisation_id = ?'),
    ...
    it('should validate WW Admin global access', async () => {
      ...
      const hasGlobalAccess = await databaseService.validateWWAdminAccess(adminUserRole.user_id);

      expect(mockDb.getFirstAsync).toHaveBeenCalledWith(
        expect.stringContaining("SELECT * FROM local_user_roles WHERE user_id = ? AND role = 'ww_admin'"),
  ...
  describe('Project Data with Organisation Scoping', () => {
    ...
    it('should prevent cross-organisation project access', async () => {
      ...
      const projects = await databaseService.getProjectsByOrganisation('different-org-456');

      expect(mockDb.getAllAsync).toHaveBeenCalledWith(
        expect.stringContaining('SELECT * FROM local_projects WHERE organisation_id = ?'),
        ['different-org-456']
      );
  ...
  describe('Offline Queue System', () => {
    ...
    it('should retrieve pending queue items by priority', async () => {
      ...
      expect(mockDb.getAllAsync).toHaveBeenCalledWith(
        expect.stringContaining("SELECT * FROM offline_queue WHERE status = 'pending' ORDER BY priority DESC, created_at ASC")
      );
