...
async function testDashboardFeatures() {
    ...
    const endpoints = [
        'dashboard', 'metrics', 'streams', 'tasks',
        'agents', 'quality-gates', 'mobile/status', 'backend/status'
    ];
    ...
    try {
        ...
        console.log(`   ⏱️  Current Velocity: ${metrics.metrics.current_velocity} tasks/day`);
        console.log(`   📋 Tasks Status: ${metrics.metrics.completed_tasks}/${metrics.metrics.total_tasks} complete`);
        console.log(`   🕐 Hours Tracked: ${metrics.metrics.hours_completed}h / ${metrics.metrics.total_estimated_hours}h`);
        console.log(`   📊 Quality Score: ${metrics.metrics.quality_score}%`);
        console.log(`   🎯 Schedule Variance: ${metrics.metrics.average_task_variance}%`);
    } catch (error) {
