...
const CONFIG = {
    ...
    get mvp2TasksDir() {
        return path.join(this.projectRoot, 'project-context/development-context/MVP2/tasks');
    }
};
...
async function loadMvp2Tasks() {
    try {
        ...
        const taskFiles = files.filter(f => f.endsWith('.txt') && f.startsWith('task_'));
        const tasks = [];
...
app.get('/api/tasks', async (req, res) => {
    try {
        ...
        const taskmasterTasks = taskmasterResult.master?.tasks || taskmasterResult.tasks || [];
...
app.get('/api/tasks/:id', async (req, res) => {
    try {
        ...
        res.status(500).json({ error: 'Failed to load task', message: error.message });
    }
});
...
function setupFileWatcher() {
    ...
    watcher.on('change', async (filePath) => {
        console.log(`📄 Task file changed: ${path.basename(filePath)}, reloading...`);
    ...
    console.log(`👁️  Watching ${watchedPaths.length} task file path(s) for changes...`);
}
...
async function startServer() {
    ...
    const [taskmasterResult, mvp2Result] = await Promise.all([
        loadTasks().catch(err => console.log('⚠️  TaskMaster tasks not available:', err.message)),
        loadMvp2Tasks().catch(err => console.log('⚠️  MVP2 tasks not available:', err.message))
    ]);
