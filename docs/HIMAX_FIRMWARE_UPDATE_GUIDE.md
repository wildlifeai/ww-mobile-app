...
|-------------|------|---------|
| **Engineer Console** | `useEngineerConsoleActions.ts` → `FirmwareStatusScreen.tsx` | Standalone status screen allowing targeted updates |

### Engineer Console Flow (`FirmwareStatusScreen.tsx` -> `HimaxFirmwareUpdateScreen.tsx`)
...
The deployment flow (`useStartDeployment.ts`) previously called `aifirmware` without a filename argument, relying on an implicit default. This has been fixed — the filename parameter is now **required**. Note: The automated Himax update has since been removed from the deployment flow in favor of targeted updates via the Engineer Console (`FirmwareStatusScreen`).
...
| `src/screens/Devices/FirmwareStatusScreen.tsx` | Entry point for triggering the update |
