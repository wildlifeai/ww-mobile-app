...
class ConfigurableDependencyValidator {
	...
	validateRules() {
		...
		Object.entries(this.config.rules || {}).forEach(([pkg, rule]) => {
			...
			if (rule.installed && installed === rule.installed) {
				console.log(
					`ℹ️  ${pkg}@${installed} - known deviation from ${rule.required}`,
				)
				if (rule.reason) {
	...
	runExpoDoctor() {
		...
		try {
			...
			if (hasUnexpectedWarnings) {
				this.warnings.push(
					'⚠️  expo-doctor reported unexpected issues - run "npx expo-doctor" to see details',
				)
			} else {
				console.log(
					"ℹ️  expo-doctor reported expected warnings (known deviations)",
				)
			}
		}
	...
	versionSatisfies(installed, requirement) {
		...
		try {
			...
			console.debug(
				`Semver parse failed for ${installed} vs ${requirement}, using string match`,
			)
		}
