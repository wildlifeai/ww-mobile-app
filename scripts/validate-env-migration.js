...
const configUsagePatterns = ["Config\\.", "Constants\\.expoConfig\\.extra\\."]
...
console.log(
	`✓ Direct expo-constants usage: ${expoConstantsUsage.length} files\n`,
)
...
if (rnConfigImports.length > 0) {
	...
	console.log(
		"✅ Environment Variable Migration COMPLETE - No react-native-config imports found!\n",
	)
}
...
try {
	...
	if (fs.existsSync(appConfigPath)) {
		...
		if (appConfigContent.includes("extra:")) {
			...
			console.log(
				"  ❌ app.config.js missing extra field for environment variables",
			)
			hasErrors = true
		}
	} else {
...
console.log(
	`Environment utility: ${
		fs.existsSync(envUtilPath) ? "✅ Present" : "❌ Missing"
	}`,
)
console.log("━".repeat(50))
