...
class PostInstallHelper {
  ...
  detectNewPackages() {
    ...
    const newPackages = currentPackages.filter(pkg => !previousPackages.includes(pkg));
    
    // Filter out packages that already have rules
    const managedPackages = Object.keys(this.config.rules || {});
    const unmanagedNewPackages = newPackages.filter(pkg => !managedPackages.includes(pkg));
  ...
  getPreviousPackages() {
    ...
    try {
      const lastSnapshot = JSON.parse(fs.readFileSync(this.lastPackagesPath, 'utf8'));
  ...
  offerToAddRules(newPackages) {
    ...
    console.log(`📦 Detected ${newPackages.length} new package(s) without rules:\\n`);
...
if (require.main === module) {
  ...
  try {
    ...
    console.error('⚠️  Post-install helper encountered an issue:', error.message);
  }
}
