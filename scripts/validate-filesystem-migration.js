...
console.log(`✓ expo-file-system usage found: ${expoFileSystemUsageFound.length} files\n`);
...
if (rnfsUsageFound.length > 0) {
  ...
  console.log('✅ File System Migration COMPLETE - No react-native-fs usage found!\n');
}

if (expoFileSystemUsageFound.length > 0) {
  console.log('📁 Files using expo-file-system:\n');
  const uniqueFiles = [...new Set(expoFileSystemUsageFound.map(item => item.file))];
...
console.log(`Files with expo-file-system: ${[...new Set(expoFileSystemUsageFound.map(item => item.file))].length}`);
