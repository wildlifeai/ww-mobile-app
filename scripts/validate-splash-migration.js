...
console.log(`✓ expo-splash-screen usage found: ${[...new Set(expoSplashUsage.map(item => item.file))].length} files\n`);

if (oldSplashUsage.length > 0) {
  console.log('❌ MIGRATION INCOMPLETE - Found react-native-bootsplash usage:\n');
  oldSplashUsage.forEach(({ file, pattern }) => {
    console.log(`  - ${file} (pattern: ${pattern})`);
  });
  console.log('\n');
} else {
  console.log('✅ Splash Screen Migration COMPLETE - No react-native-bootsplash usage found!\n');
}
...
try {
  ...
  if (fs.existsSync(appConfigPath)) {
    ...
    if (appConfigContent.includes('splash:')) {
      ...
      if (appConfigContent.includes('image:') && appConfigContent.includes('resizeMode:')) {
...
console.log(`Files with expo-splash-screen: ${[...new Set(expoSplashUsage.map(item => item.file))].length}`);
