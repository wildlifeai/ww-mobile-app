...
class DependencyRulesManager {
  ...
  async addMissingPackages() {
    ...
    const selection = await this.question('Enter numbers (e.g., 1,3,5 or "all"): ');
  ...
  async addRuleForPackage(packageName) {
    ...
    versionSuggestions.forEach((suggestion, index) => {
      console.log(`${index + 1}. ${suggestion.format} - ${suggestion.description}`);
    });
    
    const versionChoice = await this.question(`Choose version format (1-${versionSuggestions.length}) or enter custom: `);
    let requiredVersion;
  ...
  async modifyRuleProperties(packageName, rule) {
    ...
    switch (choice) {
      case '1':
        const newVersion = await this.question(`New required version (current: ${rule.required}): `);
      ...
      case '3':
        const newReason = await this.question(`New reason (current: "${rule.reason || 'none'}"): `);
        if (newReason.trim()) {
      ...
      case '4':
        const optionalChoice = await this.question(`Optional? (current: ${rule.optional || false}) (y/N): `);
        rule.optional = optionalChoice.toLowerCase().startsWith('y');
        if (!rule.optional) delete rule.optional;
        break;
        
      case '5':
        const installedVersion = await this.question('Installed version (for known deviations): ');
  ...
  async removeRuleProperty(rule) {
    ...
    const choice = await this.question(`Select property to remove (1-${properties.length}): `);
  ...
  async removeRule() {
    ...
    const packageName = packages[packageIndex];
    const confirm = await this.question(`❗ Remove rule for ${packageName}? (y/N): `);
