    ...
    IF _deployments_updated IS NOT NULL THEN
        ...
        LOOP
            ...
            SET
                ...
                device_eui = _item->>'device_eui',
                lorawan_registration_completed = COALESCE(NULLIF(_item->>'lorawan_registration_completed', '')::boolean, lorawan_registration_completed),
                lorawan_last_verified_at = NULLIF(_item->>'lorawan_last_verified_at', '')::timestamptz,
                ai_model_id = NULLIF(_item->>'ai_model_id', '')::uuid,
                ble_firmware_id = NULLIF(_item->>'ble_firmware_id', '')::uuid,
                himax_firmware_id = NULLIF(_item->>'himax_firmware_id', '')::uuid,
                battery_level_at_start = NULLIF(_item->>'battery_level_at_start', '')::int,
                sd_card_total_kb_at_start = NULLIF(_item->>'sd_card_total_kb_at_start', '')::int,
                sd_card_available_kb_at_start = NULLIF(_item->>'sd_card_available_kb_at_start', '')::int,
                lorawan_rssi_at_start = NULLIF(_item->>'lorawan_rssi_at_start', '')::int,
