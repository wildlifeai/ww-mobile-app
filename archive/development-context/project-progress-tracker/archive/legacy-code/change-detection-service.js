...
class ChangeDetectionService extends EventEmitter {
	constructor(options = {}) {
		...
		this.config = {
			baseDir:
				options.baseDir ||
				"/home/adarsh/dev/wildlifeai/wildlife-watcher-mobile-app/project-context",
	...
	async init() {
		try {
			...
			console.log(
				`✅ Change Detection Service initialized - monitoring ${this.fileStates.size} files`,
			)
		} catch (error) {
	...
	async performInitialScan() {
		...
		for (const relativePath of this.monitoredFiles) {
			...
			try {
				if (await this.fileExists(fullPath)) {
					...
					if (
						relativePath.includes("tasks/task_") &&
		...
		console.log(
			`📊 Initial scan completed: ${scannedCount} files in ${
				Date.now() - startTime
			}ms`,
		)
	...
	startMonitoring() {
		...
		console.log(
			`🚀 Starting change monitoring (interval: ${this.config.checkInterval}ms)`,
		)
	...
	async checkForChanges() {
		...
		for (const [filePath, oldState] of this.fileStates.entries()) {
			try {
				if (await this.fileExists(filePath)) {
					...
					// Compare states
					const change = await this.compareFileStates(
						filePath,
						oldState,
						newState,
					)
		...
		if (hasNewChanges) {
			...
			console.log(
				`🔄 Detected ${changes.length} changes in ${Date.now() - checkStart}ms`,
			)
		}
	...
	async extractTaskState(filePath) {
		try {
			...
			console.warn(
				`⚠️  Could not extract task state from ${filePath}:`,
				error.message,
			)
		}
	}
	...
	async detectTaskStatusChange(filePath) {
		...
		try {
			...
			console.warn(
				`⚠️  Could not detect status change for ${taskId}:`,
				error.message,
			)
		}
	...
	getStatus() {
		return {
			...
			changesSince: this.currentState.lastChangeTime
				? this.currentState.lastChangeTime.toISOString()
				: null,
	...
	getChangesSummary(since = null) {
		...
		if (since) {
			...
			changes = changes.filter(
				(change) => new Date(change.timestamp).getTime() > sinceTime,
			)
		}
	...
	addMonitoredFile(relativePath) {
		if (!this.monitoredFiles.includes(relativePath)) {
			...
			this.getFileState(fullPath)
				...
				.catch((error) => {
					console.warn(
						`⚠️  Could not add file to monitoring: ${relativePath}`,
						error.message,
					)
				})
		}
