...
| 3-6s stall on disconnect | `commandQueue` and `runCommand` lack disconnect awareness | Commands time out one-by-one instead of failing fast. See V3/V4 in Disconnect Resilience. |

---

## Disconnect Resilience

Unexpected BLE disconnects (user navigation, device DPD, signal loss) can leave multiple pipeline layers in an inconsistent state. This section documents the known vulnerability points and their mitigations.

> [!IMPORTANT]
> The file transfer pipeline (`runFileTransferPipeline.ts`) is the **gold standard** for disconnect handling — it subscribes to `BleManagerDisconnectPeripheral` directly and rejects immediately. The regular command pipeline should follow the same pattern.
...
| Rx Router | `rxRouter.clearBuffer()` called on disconnect event | `useBleListeners.tsx:186` |
| Native (Android) | `removePeripheral` awaited to ensure GATT cleanup | `useBleListeners.tsx:189-191` |
| Connect | 500ms Android GATT delay before `BleManager.connect()` | `useBle.ts:181-186` |
| Connect | Stale `isDeviceReconnecting` guard auto-cleared | `useBle.ts:145-159` |
| File Transfer | Native disconnect listener → immediate reject | `runFileTransferPipeline.ts:234-258` |

### Known Gaps (Not Yet Implemented)

| ID | Layer | Gap | Impact |
|---|---|---|---|
| V3 | `runCommand` | No disconnect signal subscription; waits for 3s timeout | 3-6s stall per command on disconnect |
| V4 | `commandQueue` | No `DISCONNECT` handler; queued commands time out sequentially | `3s × N` total hang time |
| V5 | `createBleSession` | Snapshot-based peripheral doesn't reflect live disconnect state | Writes attempted to dead device |

**Recommended fix for V3+V4:** Add `DISCONNECT` to the `DeviceSignal` enum in `deviceSignals.ts`. Emit it from the disconnect event handler in `useBleListeners.tsx`. Handle it in `commandQueue.handleDeviceSignal()` → `clearAll()` and in `runCommand` → `idempotentReject()`. Make `DEVICE_DISCONNECTED` non-retryable in `runCommandPipeline.isRetryable()`.
